<template>
  ...
  <div class="alumno-pago-view" v-if="!cargando && alumno">
    <div class="header-detalle">
      <div>
        <h1>Recibir Pago de:</h1>
     ...
     <div class="card filtro-ciclo">
      <label for="ciclo-escolar">Selecciona el ciclo para ver sus adeudos:</label>
    ...
    <div class="card">
      ...
      <form @submit.prevent="crearNuevoCargo" class="form-grid-cargo">
      ...
      <form @submit.prevent="registrarPago" class="form-grid-pago">
        ...
        <div class="full-width cargos-section">
            ...
            <div v-for="cargo in cargosPendientes" :key="cargo.id" class="cargo-a-pagar">
               <input type="checkbox" :id="`cargo-${cargo.id}`" v-model="cargosSeleccionados" :value="cargo.id" />
               <label :for="`cargo-${cargo.id}`">
                 {{ cargo.nombre_concepto }} - <strong>Saldo: ${{ cargo.saldo_pendiente }}</strong>
               </label>
            </div>
        </div>

        <button type="submit" class="full-width">Registrar y Generar Recibo</button>
      </form>
    </div>
  </div>
</template>
...
const registrarPago = async () => {
  ...
  try {
    ...
    router.push(`/recibos/${reciboCreado.id}`);
  } catch (error) {
    const errorMessage = error.response?.data?.error || 'Error desconocido';
...
<style scoped>
.alumno-pago-view { max-width: 900px; margin: 0 auto; }
